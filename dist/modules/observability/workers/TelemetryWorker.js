"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var TelemetryWorker_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.TelemetryWorker = void 0;
const common_1 = require("@nestjs/common");
const TelemetryEngine_1 = require("../engine/TelemetryEngine");
const MetricsEngine_1 = require("../engine/MetricsEngine");
let TelemetryWorker = TelemetryWorker_1 = class TelemetryWorker {
    telemetryEngine;
    metricsEngine;
    logger = new common_1.Logger(TelemetryWorker_1.name);
    constructor(telemetryEngine, metricsEngine) {
        this.telemetryEngine = telemetryEngine;
        this.metricsEngine = metricsEngine;
    }
    async processTelemetryBatch(events) {
        this.logger.debug(`Processing telemetry batch of ${events.length} events`);
        for (const event of events) {
            this.telemetryEngine.trackEvent(event);
            this.metricsEngine.incrementCounter('telemetry_events_processed_total', { category: event.category });
        }
    }
};
exports.TelemetryWorker = TelemetryWorker;
exports.TelemetryWorker = TelemetryWorker = TelemetryWorker_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [TelemetryEngine_1.TelemetryEngine,
        MetricsEngine_1.MetricsEngine])
], TelemetryWorker);
