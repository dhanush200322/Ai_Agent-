...
exports.startBillingCronJobs = void 0;
...
exports.startBillingCronJobs = startBillingCronJobs;
const processReminders = async () => {
    const now = new Date();
    // Subscriptions ending in exactly 30 days (Annual)
    const in30Days = new Date(now);
    in30Days.setDate(in30Days.getDate() + 30);
    const subs30 = await prisma_1.prisma.organizationSubscription.findMany({
        where: {
            status: 'ACTIVE',
            billingCycle: 'annual',
            expiryDate: {
    ...
    for (const sub of subs30) {
        if (sub.user?.email) {
            ...
            await prisma_1.prisma.organizationSubscription.update({
                where: { id: sub.id },
    ...
    const subs7 = await prisma_1.prisma.organizationSubscription.findMany({
        where: {
            status: 'ACTIVE',
            billingCycle: 'annual',
            expiryDate: {
    ...
    const subs3 = await prisma_1.prisma.organizationSubscription.findMany({
        where: {
            status: 'ACTIVE',
...
const processDowngrades = async () => {
    ...
    const expiredSubs = await prisma_1.prisma.organizationSubscription.findMany({
        where: {
            status: 'ACTIVE',
            expiryDate: {
                lt: now
    ...
    for (const sub of expiredSubs) {
        ...
        const starterPlan = await prisma_1.prisma.subscriptionPlan.findFirst({
