...
class ContextEngine {
    async getContext(agentId, key) {
        const context = await prisma_1.prisma.agentContext.findUnique({
    ...
    async deleteContext(agentId, key) {
        await prisma_1.prisma.agentContext.delete({
            where: { agentId_key: { agentId, key } }
        });
    }
    async getAllContext(agentId) {
        const contexts = await prisma_1.prisma.agentContext.findMany({ where: { agentId } });
        const result = {};
