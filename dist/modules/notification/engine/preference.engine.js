...
class PreferenceEngine {
    async resolvePreferences(context) {
        ...
        if (orgPref)
            resolved = { ...resolved, ...orgPref };
        if (deptPref)
            resolved = { ...resolved, ...deptPref };
        if (teamPref)
            resolved = { ...resolved, ...teamPref };
        if (userPref)
            resolved = { ...resolved, ...userPref };
        return resolved;
    }
    isChannelEnabled(resolvedPref, channel) {
        let enabled = [];
        let disabled = [];
        try {
        ...
        if (disabled.includes(channel))
            return false;
        if (enabled.length > 0 && !enabled.includes(channel))
            return false;
        return true;
    }
    isInQuietHours(resolvedPref) {
