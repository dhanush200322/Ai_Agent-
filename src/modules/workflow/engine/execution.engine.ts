import { NodeExecutionStatus } from '@prisma/client';
...
import { MeteringEngine } from '../../billing/engine/metering.engine';
          ...
          try {
            ...
            if (timeoutMs > 0) {
              result = await Promise.race([
                globalNodeRegistry.execute(node.type, node, context),
                new Promise<any>((_, reject) => 
                  setTimeout(() => reject(new Error('Timeout')), timeoutMs)
                )
              ]);
            } else {
              result = await globalNodeRegistry.execute(node.type, node, context);
            }
  ...
  async resume(context: WorkflowExecutionContext, nodeId: string, approvalData?: any): Promise<void> {
    ...
    try {
      ...
      if (approvalData?.retry) {
        ...
        if (!executor.resume) throw new Error(`Node type ${node.type} does not support resume.`);
