...
export class TemplateEngine {
  render(content: string, context: TemplateContext): string {
  ...
  renderSubject(subject: string | null | undefined, context: TemplateContext): string {
    if (!subject) return '';
    return this.render(subject, context);
  }

  applyBranding(body: string, type: NotificationTemplateType, brandingConfig: { logoUrl?: string; primaryColor?: string } = {}): string {
    if (type !== 'HTML') {
      return body;
    }
