import { AgentTask, ConsensusStrategy } from '@prisma/client';

export class ConsensusService {
  /**
   * Aggregates the results of multiple subtasks into a cohesive response.
   */
  async aggregate(_parentTask: AgentTask, subtasks: AgentTask[], strategy: ConsensusStrategy): Promise<string> {
    if (subtasks.length === 0) return 'No subtasks executed.';
  ...
  private formatOutputs(tasks: AgentTask[]): string {
